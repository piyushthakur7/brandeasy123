          ...
          {collections.map((s) => (
            <motion.div key={s.title} variants={itemVariants}>
              <Link href="/shop" className="block h-full">
                <div className="group bg-white rounded-3xl p-8 cursor-pointer hover:shadow-2xl hover:shadow-[#0099D4]/10 transition-all duration-500 relative h-full flex flex-col items-start border border-slate-100">
                  {s.tag && (
                    <span className="absolute top-4 right-4 text-[8px] bg-[#0099D4] text-white font-black px-3 py-1 tracking-widest uppercase rounded-full shadow-lg">
                      {s.tag}
                    </span>
                  )}
